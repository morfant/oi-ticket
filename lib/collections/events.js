...
Meteor.methods({
  ...
  searchInName(keyWord) {
    ...
    try {
      ...
      for (var i = 0; i < rslt.length; i++) {
        ...
        var post = Posts.findOne(rslt[i].post_ID);
        rslt[i].playTitle = post.title;
        rslt[i].state = post.state;
  ...
  // searchInPhoneNumber(keyWord, post_state_int) {
