...
validatePhoneNumber = function (number) {
  ...
  if (re.test(info.mobileNumber))
    errors.reserve_mobilePhone = "Only Number is allowed.";

  if (number.length > 11)
    errors.reserve_mobilePhone = "Too long to be a phone number.";
  return errors;
...
validateReservInfo = function (info) {
  ...
  if (!info.name)
    errors.reserve_name = "Please fill in a name";

  if (!info.mobileNumber)
    errors.reserve_mobilePhone = "Please fill in a moblile number";

  if (!info.seats)
    errors.reserve_seats = "Please fill in a seats";
